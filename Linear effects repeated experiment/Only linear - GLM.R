##############################################################
## Experiment with only linear effects - GLM
##############################################################
...
for (l in 1:L) {
  ...
  while (n_iter < max_iter && !identical(get(paste0("Nij_",n_iter+1)),get(paste0("Nij_",n_iter))) ) {
    ...
    param_list = vector("list", d - 1)
    preds_q_list   = vector("list", d - 1)
    ...
    for (i in 1:(d-1)) {
      ...
      temp = optim(
        par = init_params,
        fn = function(params) {
          likelihood_function_GLM_vectorised(params = params, data = x, Nij_current = Nij_current, q_index = i)
        },
        method = "BFGS"
      )
      
      param_list[[i]]   = temp$par
      preds_q_list[[i]] = as.matrix(x) %*% temp$par
      
    }
    
    assign(paste0("param_rep_",n_iter), param_list)
    q_matrix = do.call(cbind, preds_q_list)
    ...
    preds_val_q_list = lapply(1:(d - 1), function(i) {
      as.matrix(x_val) %*% param_list[[i]]
    })
    
    q_matrix_val = do.call(cbind, preds_val_q_list)
  ...
  preds_test_q_list = lapply(1:(d - 1), function(i) {
    as.matrix(x_TEST) %*% model_rep[[i]]
  })
  
  q_matrix_test = do.call(cbind, preds_test_q_list)
